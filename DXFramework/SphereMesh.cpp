...
void SphereMesh::initBuffers(ID3D11Device* device)
{
	...
	for (int counter = 0; counter < v; counter++)
	{
		...
		//change texture co-ordinates so that they are mapped to the sphere correctly
		longd = atan2(vertices[counter].normal.x, -vertices[counter].normal.z);
		lat = acos(vertices[counter].normal.y);

		vertices[counter].texture.x = (longd / (XM_PI * 2.0) + 0.5);
		vertices[counter].texture.y = (lat / XM_PI);
	}
...
void SphereMesh::sendData2(ID3D11DeviceContext* deviceContext)//changes data structure for tesselation
{
	unsigned int stride;
	unsigned int offset;
